...
TIMEFRAME = None if IS_MAC else mt5.TIMEFRAME_H1
...
def fetch_data(symbol):
    ...
    if IS_MAC:
        cache_file = get_cache_filename(symbol)
        print(f"  [ERROR] No cached data found for {symbol}.")
        print(f"          Expected cache file: {cache_file}")
        print(f"          On macOS, MetaTrader5 is not supported. Please provide the cache file.")
        return None
...
def generate_signal(row):
    ...
    if row['adx'] < 15:
        return 0  # no trend (range market)

    # EMA200 major trend filter — only trade with the big trend
    if buy >= 3 and row['close'] > row['ema200']:
        return 1
    if sell >= 3 and row['close'] < row['ema200']:
        return -1
    return 0
...
def main():
    ...
    if IS_MAC:
        print("[INFO] macOS detected — MetaTrader5 not supported. Reading from cache only.")
    elif not connect_mt5():
        return
